...
class IOauth2Setting(form.Schema, IImageScaleTraversable):
    ...
    """
    twitterAppId = schema.TextLine(
        title=_(u"Twitter app id"),
        required=False,
    )

    twitterAppSecret = schema.TextLine(
        title=_(u"Twitter app secret"),
        required=False,
    )

    twitterScope = schema.TextLine(
        title=_(u"Twitter auth scope"),
        required=False,
    )

    twitterRedirectUri = schema.URI(
        title=_(u"Twitter redirect URL"),
        required=False,
    )"""
